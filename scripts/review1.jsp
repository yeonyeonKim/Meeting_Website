...
<body>
	...
	<%
		...
		String rComName = "", rComCon = "", rComDate = "", rcomment; //comment
	 ...
	 <!-- comment -->
         ...
         <div align="left">
            <h3>[댓글 작성]</h3>
            <textarea id="rcomment" name="rcomment" cols="150" rows="3" placeholder="댓글을 입력해주세요"></textarea>&nbsp;
            <%
               rcomment = request.getParameter("rcomment");
            %>
            <input type="hidden" name="rcomment" value="<%=rcomment%>">
            <%
            String able_sql="select distinct Review.rv_id as attend from (((Review left outer join Meeting on Meeting.me_id=Review.me_id)left outer join User as user1 on user1.us_id=Review.us_id)join User as user2)left outer join Acceptance on Acceptance.me_id=Review.me_id where (Meeting.us_id=user2.us_id or (Acceptance.us_id=user2.us_id and Acceptance.ac_yesno='y')) and user2.us_id='"+id+"'";
            rs = stmt.executeQuery(able_sql);
            int attendmeeting = 0;
            while(rs.next()){
            attendmeeting=Integer.parseInt(rs.getString("attend"));
            if(attendmeeting == rv_id){
            	cbuttonable="abled";
            	break;
            }
            else cbuttonable="disabled";
            }
            ...
            %>
            <input type="hidden" name="comment_rv_id" value="<%=rv_id%>">
